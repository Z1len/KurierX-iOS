...
struct FirebaseSetupView: View {
    var body: some View { ContentUnavailableView("Firebase не настроен", systemImage: "flame.fill", description: Text("Добавьте GoogleService-Info.plist и пересоберите KurierX.")) }
}

struct BlockedView: View {
    let title: String; let text: String
    var body: some View { ContentUnavailableView(title, systemImage: "lock.fill", description: Text(text)) }
}
